...
class Maestros extends Conexion{
	...
	#INGRESO USUARIO
	public function ingresoMaestroModel($datosModel){

		$admin = array('email' => 'admin', 'password' => 'admin', 'nombre' => 'ADMINISTRADOR');

		if($datosModel['email'] == $admin['email'] && $datosModel['password'] == $admin['password']){
			return $admin;
		}
		else{
			$stmt = Conexion::conectar()->prepare("SELECT * FROM MAESTROS WHERE email = :email and password = :password");	
